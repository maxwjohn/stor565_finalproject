...
pca_health <- prcomp(latest_filtered_clean[3:ncol(latest_filtered_clean)], scale = TRUE)
...
#NOTE this code clusters the cities
```{r}
...
#Kmeans function on the Original Data (k=2:17) #NOTE this code clusters the cities
```{r}
...
#Now lets preform k_means on the scaled data to avoid distribution related errors #NOTE this code clusters the cities
```{r}
...
#K_means on PCA components ###Need to do this on 10 Principal components
...
Now we are going to cluster the feature variable with K-means 
```{r}
performKMeansRange(t(scale(latest_data_num)), k_range)
...
```{r}
#K-means on transposed PCA
k_range = 2:17
performKMeansRange(t(pca_health$x), k_range)
...
Get a table with cities and what cluster they are in (Optimal Clusters are 3,4,5)

Find the feature means for each cluster (analyse) (looking for health impacts) / if difficult you can do k-means on the original data

PCA k-means on first 10 principle components

Analyse principal components for meaning 


Run all your code on Max's reduced Dataframe in branch Start.rmd (run all the code) in outline it is in "final variables for clustering"



Using new_df (dataframe without highly correlated variables)
```{r}
new_df = (read.csv("new_df.csv"))

scale_new_df = scale(new_df[,2:ncol(new_df)])
#kmeans
colnames(scale_new_df)
# Assuming 'latest_data_num' is your prepared data matrix or data frame

# Perform k-means clustering with k clusters
k <- 4  # Specify the number of clusters
kmeans_result <- kmeans(scale_new_df[,!colnames(scale_new_df) %in% c("date_label")], centers = k)
...
```
Now lets see what k-means looks like with 3 clusters on the reduced dataset from Max's code
...
```
Finding the cities in each cluster

```{r}
#Now that we have completed K-means on the reduced data set, lets visualize the cities we found in each cluster
View(new_df)
scale_new_df = scale(new_df[,2:ncol(new_df)])
View(scale_new_df)
scale_new_df$Cluster <- cluster_assignments
#scale_new_df[, c("geo_label_city", "Cluster")]
# Ensure the first column has a name, if not already named (replace 'ID' with your chosen name)
names(new_df)[1] <- "ID"

# Extract rows for each cluster and their identifiers
cluster1 <- new_df[new_df$Cluster == 1, "ID"]
cluster2 <- new_df[new_df$Cluster == 2, "ID"]
cluster3 <- new_df[new_df$Cluster == 3, "ID"]

# Optionally, print the IDs in each cluster
print(cluster1)
print(cluster2)
print(cluster3)
...
#Code for final project  (4/22/2024)
```{r}
#pca_health$x
k = 4
kmeans_result <- kmeans(pca_health$x[,1:12], centers = k)

pca_cluster = data.frame((kmeans_result$cluster), (latest_filtered_clean[1]))

View(pca_cluster)

write.csv(pca_cluster, "K_means_Cluster_4.csv", row.names = FALSE)
...
write.csv(pca_cluster, "K_means_Cluster_3.csv", row.names = FALSE)

```

Now I will plot the cities onto PC1 and PC2 and indicate which cluster assignment each point has based on color

```{r}
# Load necessary libraries
library(ggplot2)

# Assume pca_health contains PCA results, and 'df' is your DataFrame with city and cluster
# Let's create a new data frame that includes PCA results and the cluster information
# Extract the first two principal components
pca_df <- data.frame(PC1 = pca_health$x[,1], PC2 = pca_health$x[,2])
df = read.csv("K_means_Cluster_3.csv")
df$City = df[,2]
df$Cluster = df[,1]
# Add city and cluster information assuming 'df' has columns 'City' and 'Cluster'
pca_df$City <- df$City
pca_df$Cluster <- df$Cluster

# Create the plot with adjusted label positions and smaller font size
ggplot(pca_df, aes(x = PC2, y = PC1, color = as.factor(Cluster), label = City)) +
  geom_point() +  # Plot points
  geom_text(vjust = -.5, hjust = .5, size = 3) +  # Add city names closer to the point with smaller font size
  scale_color_manual(values = rainbow(length(unique(pca_df$Cluster)))) +  # Color by cluster using rainbow colors
  labs(title = "PCA of Health Data by City", 
...
Now I will complete the same process for k = 4
```{r}
...
# Extract the first two principal components
pca_df <- data.frame(PC1 = pca_health$x[,1], PC2 = pca_health$x[,2])
df = read.csv("K_means_Cluster_4.csv")
df$City = df[,2]
df$Cluster = df[,1]
# Add city and cluster information assuming 'df' has columns 'City' and 'Cluster'
pca_df$City <- df$City
...
```{r}
#setwd("C:/Users/maxb1/OneDrive/Documents/UNC_Stuff/stor_565/final_project")
all_cities = read_csv("BigCitiesHealth.csv")

...
# Defining a vector of distance measures to test
distance_measures <- c("euclidean", "maximum", "manhattan", "canberra", "binary", "minkowski", "pearson", "spearman", "kendall")

# Performing HC with different distance measures
results <- lapply(distance_measures, function(dist_measure) {
  if (dist_measure %in% c("pearson", "spearman", "kendall")) {
    # Computing correlation matrix
    cor_matrix <- cor(pca_scores, method = dist_measure)
    # Converting correlation matrix to dissimilarity matrix
    diss_matrix <- as.dist(1 - cor_matrix)
  } else {
    # For other distance measures, use dist function directly
    diss_matrix <- dist(pca_scores, method = dist_measure)
  }
  # Performing hc
  hc_result <- hclust(diss_matrix, method = "complete")
  return(list(distance_measure = dist_measure, dendrogram = hc_result))
})

# Printing and comparing the results
par(mfrow = c(3, 3))  #  plots in a grid
for (res in results) {
  plot(res$dendrogram, main = paste("Hierarchical Clustering with", res$distance_measure, "distance"))
}
...
```{r}
# Exclude "U.S. Total" from city names
city_names <- cities_clean$geo_label_city[-1]

# Create a data frame with city names and cluster assignments
city_cluster_df <- data.frame(
  City = city_names,
  Cluster = cluster_assignments
)

# Write the data frame to a CSV file
write.csv(city_cluster_df, file = "city_cluster_data.csv", row.names = FALSE)

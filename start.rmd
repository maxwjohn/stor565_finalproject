...
all_cities = read_csv("bigcitiesall.csv")
...
na_count = colSums(is.na(cities)) / nrow(cities))
lowna = na_count[colSums(is.na(cities)) / nrow(cities) < .33]
lowna_cols = names(lowna)
cities = cities %>% select(all_of(lowna_cols))
non_zero_sd_columns <- apply(cities, 2, sd) != 0

# Subset the dataframe to keep only columns with non-zero standard deviation
cities_filtered <- cities %>% select(names(non_zero_sd_columns))
```
...
## Numerical columns
num_cols = cities_filtered %>% select(!c(geo_label_city, geo_fips_code, date_label))
